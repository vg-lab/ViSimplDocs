...
This documentation is for ViSimpl version 1.8.1 that can be obtained from the homepage.
...
.. code-block:: bash
  ...
  xhost +local:docker
  # Pull the image.
  docker pull vglab/visimpl:1.8.1-nvidia-ubuntu-16.04
  ...
  cd ..
  # Run example
  docker run --gpus 1 -ti --rm -e DISPLAY -v /tmp/.X11-unix:/tmp/.X11-unix -v /etc/machine-id:/etc/machine-id -v $(pwd)/data:/data --privileged vglab/visimpl:1.8.1-nvidia-ubuntu-16.04 /usr/bin/visimpl.sh
...
.. code-block:: bash
  ...
  docker run --gpus 1 -ti --rm -e DISPLAY -v /tmp/.X11-unix:/tmp/.X11-unix -v /etc/machine-id:/etc/machine-id -v $(pwd)/data:/data --privileged vglab/visimpl:1.8.1-nvidia-ubuntu-16.04 bash
